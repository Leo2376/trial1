...
| C extension decompressor | Skeleton exists, unverified | — |
| A extension (LR/SC/AMO) | Stub decode only | — |
...
1. **C extension (rv64uc)** — nearest win. `decompressor.sv` exists; run the
   rv64uc suite, fix decode/imm bugs (same loop as F work).
2. **D extension (rv64ud)** — double-precision datapath functions already in
   `fpu.sv`; test-and-fix. Completes "G".
3. **A extension (rv64ua)** — LR/SC + AMO semantics in LSU; AXI exclusive /
   lock path already stubbed.
...
7. **RVV 1.0 (VLEN=256) + IME 1.0** — shared 1 MiB LLC with CPU.
...
3. **ISA regression** — `tools/scripts/run_isa_tests.sh`: compiles riscv-tests
   sources (rv64ui/rv64um/rv64uf, selectable via `EXT=`), converts ELF to hex
...
Last full regression: rv64ui 50/50, rv64um 13/13, rv64uf 11/11, tb_fpu 61/61.
...
│   ├── frontend/
│   │   └── decompressor.sv       # C-extension decompressor (skeleton)
│   ├── ctrl/
...
```
L1I (32 KiB) ─┐
              ├─→ L2 (256 KiB) ─→ LLC (1 MiB, shared CPU+VPU) ─→ AXI4
L1D (32 KiB) ─┘
VPU ldst    ─────→ L2 ─────────→ LLC
```
...
- **Vector**: RVV 1.0 (VLEN=256, ELEN=64) — planned
- **Matrix**: IME 1.0 (shared VPU regfile) — planned
...
| LLC   | 1 MiB | Shared (CPU + VPU) | planned |
